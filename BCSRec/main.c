...
void setLength(int input, int *length) {
	if (input > 0 && input < 100) { //Fixed by removing the = from input >= 0
		*length = input;
	}
}
...
void setWidth(int input, int *width) {
	if (input > 0 && input < 100) { //Fixed by removing the = from input <= 100
		*width = input;
	}
}
...
int getPerimeter(int *length, int *width) {
	int perimeter = *length + *length + *width + *width; //Fixed by setting the calculation as 2(l+w) instead of 2(l)+w
	return perimeter;
}
